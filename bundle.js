(()=>{
  ...
  (() => {
    ...
    // =========================================================================
    // 1. REDIRECCIÓN INFALIBLE DE REDES SOCIALES (open-url-button.ts)
    // =========================================================================
    const SOCIAL_LINKS = {
    ...
    function navigateToUrl(url) {
      const now = Date.now();
      if (now - lastUrlOpenTime < 800) return;
      lastUrlOpenTime = now;
    ...
    function getObjectScreenDistance(camera, obj, clientX, clientY, canvasRect) {
      ...
      try {
        obj.traverse((child) => {
          if (child.isMesh && child.geometry) {
            ...
            if (child.geometry.boundingSphere && camera.project) {
        ...
        if (!foundGeometry || minDistance === Infinity) {
    ...
    function handleTouchOnIcons(world, clientX, clientY, targetEid) {
      ...
      // Detección directa de target
      if (targetEid && entityToObject.has(targetEid)) {
        const targetObj = entityToObject.get(targetEid);
        let gltfSrc = "";
        try {
          if (ECS.GltfModel && ECS.GltfModel.has(world, targetEid)) {
            const c = ECS.GltfModel.get(world, targetEid);
            gltfSrc = c.src || c.url || "";
          }
        } catch (e) {}

        const fullStr = `${targetObj?.name || ""} ${gltfSrc}`.toLowerCase();
        if (!fullStr.includes("pedestal") && !fullStr.includes("plane") && !fullStr.includes("yobailanding")) {
          const url = getUrlFromIdentifier(fullStr);
          if (url) {
            pulseObjectScale(targetObj);
            navigateToUrl(url);
            return;
          }
        }
      ...
      for (const [eid, obj] of entityToObject.entries()) {
        ...
        const distance = getObjectScreenDistance(camera, obj, clientX, clientY, canvasRect);
        if (distance < minDistance) {
          minDistance = distance;
          closestMatch = { obj, url, dist: distance };
        }
      }
